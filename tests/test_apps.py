...
class AppsTest(ShimTestCase):
    ...
    def test_v2_get_apps(self):
        endpoint = f"/v2/apps?q=space_guid:{AppsTest.space_guid}"
        (v2_json, shim_json) = self.run_v2_shim_get(endpoint)
    ...
    def test_v2_get_apps_q_name(self):
        endpoint = f"/v2/apps?q=name:app2&q=space_guid:{AppsTest.space_guid}"
        (v2_json, shim_json) = self.run_v2_shim_get(endpoint)
        # see above
    ...
    def test_v2_get_apps_results_per_page(self):
        endpoint = f"/v2/apps?q=space_guid:{AppsTest.space_guid}&results-per-page=1"
        (v2_json, shim_json) = self.run_v2_shim_get(endpoint)
        self.assertDict(v2_json, self.tweak_apps_shim_json(v2_json, shim_json))

        endpoint = f"/v2/apps?q=space_guid:{AppsTest.space_guid}&results-per-page=1&page=2"
        (v2_json, shim_json) = self.run_v2_shim_get(endpoint)
        self.assertDict(v2_json, self.tweak_apps_shim_json(v2_json, shim_json))

        endpoint = f"/v2/apps?q=space_guid:{AppsTest.space_guid}&results-per-page=100&page=2"
        (v2_json, shim_json) = self.run_v2_shim_get(endpoint)
        # v3 returns total_results=0 for too high page numbers
        shim_json["total_results"] = v2_json["total_results"]
        self.assertDict(v2_json, self.tweak_apps_shim_json(v2_json, shim_json))

        endpoint = f"/v2/apps?q=space_guid:{AppsTest.space_guid}&q=name:app2&results-per-page=1"
        (v2_json, shim_json) = self.run_v2_shim_get(endpoint)
        self.assertDict(v2_json, self.tweak_apps_shim_json(v2_json, shim_json))
